class Instance {
  ...
  kp_ring_create (handle_ptr, ring_params_ptr, buf_ptr, buf_len) {
    ...
    const calc_len = (n) => {
      const reqs = Math.pow(2, n)
      const size = 7 * 4 + reqs * 28 + reqs * 2 * 16
      return size
    }
    let n = 0
  ...
  kp_ring_enter (ringId, min_complete, max_time) {
    ...
    const flags = this.getUint32(ring.flags, true)
    if ((flags & 0b10) === 0) {
  ...
  async run_main (memory) {
    ...
    const results = await WebAssembly.instantiate(this.wasmBytes, { env })
